...
**agent-mux** is a one-command tmux setup with a cross-pane CLI (`tmux-bridge`) designed for AI agent terminal automation. Three deliverables:

1. `.tmux.conf` — opinionated tmux config with Option-key bindings
2. `scripts/tmux-bridge` — bash CLI for reading/writing across tmux panes
...
```bash
# Check tmux-bridge connectivity
tmux-bridge doctor

# Exercise core commands
tmux-bridge list
tmux-bridge read <pane-target>
tmux-bridge type <pane-target> "text"
tmux-bridge keys <pane-target> Enter
```

## Architecture

### tmux-bridge read-guard

The core safety mechanism: an agent must call `read` before `type` or `keys`. A `/tmp/tmux-bridge-read-<pane-id>` marker file is created on read and consumed on type/keys. Bypassing this is intentional — don't weaken it.

### Target resolution

Pane targets accept: tmux natives (`%3`, `session:window.pane`, window index `0`), or custom labels set via `tmux-bridge name <target> <label>`. Labels are stored in the `@name` pane option and resolved at runtime.

### Message convention

`tmux-bridge message` auto-prepends a header:
```
[tmux-bridge from:<sender> pane:<pane-id> at:<session:window.pane> - load the agent-mux skill to reply]
```
...
Detects OS/package manager → installs tmux if missing → downloads `tmux-bridge` and `agent-mux` CLI → adds `~/.agent-mux/bin` to shell rc. With `--with-config`: additionally backs up existing config to `~/.agent-mux/backups/`, downloads `.tmux.conf`, symlinks it to `~/.config/tmux/tmux.conf`, and reloads tmux if running.
