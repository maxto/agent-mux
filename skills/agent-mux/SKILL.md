...
Both intermediate pane reads are printed to stdout. Do NOT poll or read the target pane after sending — the target agent types its reply directly into your pane via tmux-bridge.
...
```
[tmux-bridge from:claude pane:%4 at:3:0.0 - load the agent-mux skill to reply] Please review src/auth.ts
```
...
```
[tmux-bridge from:claude pane:%4 at:3:0.0 - load the agent-mux skill to reply] What is the test coverage for src/auth.ts?
```
